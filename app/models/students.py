...
def read_student(student_id):
    cursor = mysql.connection.cursor(dictionary=True)
    query = "SELECT student.id, student.firstname, student.lastname, student.yearlevel, student.gender, student.image_url, course.coursecode, course.coursename, college.collegecode, college.collegename FROM student JOIN course ON student.coursecode = course.coursecode JOIN college ON course.collegecode = college.collegecode WHERE student.id = %s"
    cursor.execute(query, (student_id,))
    result = cursor.fetchone()
    cursor.close()
    return result
